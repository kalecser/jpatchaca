...
public class PeriodsList extends SimpleInternalFrame implements TaskSelectionListener {
	...
	public PeriodsList(final TaskList tasksList, final PeriodsInTasksSystem periodsInTasks, final PeriodsTableModel periodsTableModel, final SwingTasksUser tasksUser, final BasicSystem basic){
		...
		removePeriodAlert().subscribe(new Subscriber() {
			public void fire() {
				model.removePeriod(tasksList.selectedTask(), selectedPeriodIndex());
			}			
		});		
	...
	private JScrollPane getPeriodsTable() {
		...
		this.periodsTable.getSelectionModel().setSelectionMode(ListSelectionModel.MULTIPLE_INTERVAL_SELECTION);
	...
	private void adjustColumnsAppearance(final JXTable table) {
		...
		columnModel.getColumn(3).setCellRenderer(new DefaultTableRenderer( StringValue.TO_STRING, SwingConstants.RIGHT));
	}
	...
	void selectAndScrollToRow(final int row) {
		...
		this.periodsTable.scrollRectToVisible(this.periodsTable.getCellRect(row,0,true));
	}
	...
	public void setSelectedPeriodTesting(final int index) {
		this.periodsTable.getSelectionModel().setSelectionInterval(index, index);
	}
