...
public class PeriodsInTasksSystemImpl implements PeriodsInTasksSystem, Startable {
	...
	public void addPeriod(final TaskView task, final Period period) {
		...
		final AddPeriodEvent addPeriod = new AddPeriodEvent(idOfTask, period.startTime(), period.endTime());
	...
	public void editPeriodDay(final TaskView task, final int i, final Date date) {
		...
		final Date adjustedStartTime = copyDayMonthYear(period.startTime(), date);
	...
	private Date copyDayMonthYear(final Date datea, final Date dateb) {
		...
		calendarc.set(Calendar.DAY_OF_YEAR, calendarb.get(Calendar.DAY_OF_YEAR));
		calendarc.set(Calendar.HOUR_OF_DAY, calendara.get(Calendar.HOUR_OF_DAY));
	...
	public void removePeriod(final TaskView task, final Period period) {
		...
		eventsSystem.writeEvent(
				new RemovePeriodEvent(	idOfTask, 
										indexOfPeriod));
